...
```cpp
vecotor<string> vecStr = {"Value1", "Value2"};
```
...
**在一般情况下，可以将这三种初始化类型都看作值初始化。其一般规则为，基本内置类型为0，`string`类型为空串。**

值初始化用于有初始化器但初始化器为空时的情形。如

```cpp
vecotor<int>Vec(10); //vec中有10个被执行值初始化的int型变量
int i = new int();
```

`zero`初始化没有显式调用的语法，在某些值初始化的情况下会调用`zero`初始化。
...
对于聚合类型和数组来说，在有初始化器但初始化器为空时的情形下，执行的是聚合类型初始化而不是值初始化。
